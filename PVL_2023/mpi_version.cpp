...
int main(int argc, char** argv) {
    ...
    int extra = grid_size % world_size;
    int local_rows;
    if (world_rank < extra) {
    ...
    int row_start = world_rank * rows_per_rank + std::min(world_rank, extra);

    // Random number generation
    std::mt19937 rng(std::chrono::steady_clock::now().time_since_epoch().count() + world_rank);
    ...
    // Initialize a few infected individuals (only root, then scatter)
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        if (world_rank == 0) flat_gathered_grid.resize(grid_size * grid_size);
        for (int i = 0; i < local_rows; ++i)
            for (int j = 0; j < grid_size; ++j)
        ...
        if (local_rows > 0) {
            ...
            if (world_rank > 0) {
                ...
                MPI_Sendrecv(top_row.data(), grid_size, MPI_INT, world_rank - 1, 0,
                             recv_top.data(), grid_size, MPI_INT, world_rank - 1, 1,
            ...
            if (world_rank < world_size - 1) {
                ...
                MPI_Sendrecv(bottom_row.data(), grid_size, MPI_INT, world_rank + 1, 1,
                             recv_bottom.data(), grid_size, MPI_INT, world_rank + 1, 0,
                             MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
            ...
            for (int j = 0; j < grid_size; ++j) {
                if (grid[i][j] == INFECTED) {
                    for (int d = 0; d < directions.size(); d++) {
                        ...
                        } else if (ni == -1 && world_rank > 0 && nj >= 0 && nj < grid_size) {
                            neighbor_val = recv_top[nj];
                        } else if (ni == local_rows && world_rank < world_size - 1 && nj >= 0 && nj < grid_size) {
